    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
            ...
            <nav class="flex-1 overflow-y-auto px-4 py-6 space-y-1 custom-scrollbar">
                <p class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] px-6 mb-2">Main Menu</p>
                ...
                <p class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] px-6 mb-2 mt-6">Management</p>
            ...
            <div class="p-4 border-t border-slate-800/50 bg-slate-900/50">
                <div class="flex items-center gap-3 px-4 py-3 bg-slate-800/40 rounded-2xl border border-slate-700/50">
                    <div class="w-8 h-8 rounded-lg bg-red-600 flex items-center justify-center text-xs font-black uppercase">
                        {{ substr(Auth::user()->name ?? 'A', 0, 1) }}
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs font-bold truncate">{{ Auth::user()->name ?? 'Admin' }}</p>
                        <p class="text-[9px] text-slate-500 font-bold uppercase tracking-tighter">Super Admin</p>
                    </div>
                </div>
            </div>
        </aside>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header class="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-6 lg:px-10 shrink-0">
                <button @click="sidebarOpen = true" class="lg:hidden p-2.5 bg-slate-100 rounded-xl text-slate-600 active:scale-95 transition-all">
                    <i data-lucide="menu" class="w-6 h-6"></i>
                </button>

                <div class="flex items-center gap-2">
                    <span class="hidden md:inline-flex items-center gap-2 px-4 py-2 bg-green-50 text-green-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-green-100">
                ...
                <div class="relative" x-data="{ profileOpen: false }">
                    <button @click="profileOpen = !profileOpen" @click.away="profileOpen = false" 
                        ...
                        <div class="text-left hidden sm:block">
                            ...
                            <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Manage Account</p>
                        </div>
                    ...
                    <div x-show="profileOpen" x-cloak
                        ...
                        <div class="px-4 mt-2">
                            <form action="{{ route('admin.logout') }}" method="POST">
                                @csrf
                                <button class="flex items-center gap-3 w-full px-4 py-3 bg-red-50 text-red-600 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all">
