...
<div class="min-h-screen bg-[#F8F9FA] pb-32 font-sans" x-data="{ activeTab: 1 }">
    ...
    <div class="bg-slate-900 pt-8 pb-20 px-6 rounded-b-[2.5rem] shadow-xl relative overflow-hidden text-center">
        ...
        <div class="mt-8 grid grid-cols-3 gap-3 relative z-10">
            ...
            <div class="bg-white/5 backdrop-blur-md p-3 rounded-2xl border border-white/10">
                ...
                <p class="text-[15px] font-black text-white mt-1.5">₹{{ number_format($totalCommission) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="px-5 -mt-10 relative z-20 space-y-4">
        ...
        <div class="space-y-3">
            ...
            <div x-show="activeTab === 1" class="space-y-3" x-transition.opacity>
                ...
                @empty
                    <div class="text-center py-10"><p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">No Level 1 Members</p></div>
                @endforelse
            </div>
            ...
            <div x-show="activeTab === 2" class="space-y-3 hidden" x-transition.opacity>
                @forelse($level2 as $member)
                    <x-team-member-card :member="$member" />
                @empty
                    <div class="text-center py-10"><p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">No Level 2 Members</p></div>
                @endforelse
            </div>
            ...
            <div x-show="activeTab === 3" class="space-y-3 hidden" x-transition.opacity>
                @forelse($level3 as $member)
                    <x-team-member-card :member="$member" />
                @empty
                    <div class="text-center py-10"><p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">No Level 3 Members</p></div>
                @endforelse
            </div>
